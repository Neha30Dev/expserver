...
xps_session_t *xps_session_create(xps_core_t *core, xps_connection_t *client) {
  ...
  session->client_source->active = true;
  session->client_sink->active = true;
  session->upstream_source->active = true;
  session->upstream_sink->active = true;
  session->file_sink->active = true;
  session->lookup = NULL;
...
void session_process_request(xps_session_t *session) {
  ...
  if (lookup->type == REQ_FILE_SERVE) {
    if (lookup->file_path) {
      ...
      if (upstream == NULL) {
          logger(LOG_ERROR, "session_process_request()", "upstream connect failed");
          res = xps_http_res_create(session->core, HTTP_BAD_GATEWAY);
          xps_buffer_t *buff = xps_http_res_serialize(res);
          set_to_client_buff(session, buff);
          xps_http_res_destroy(res);
          return;
      ...
      res = xps_http_res_create(session->core, lookup->http_status_code);
      xps_http_set_header(&res->headers, "Location", lookup->redirect_url);
      xps_buffer_t *http_res_buff = xps_http_res_serialize(res);
