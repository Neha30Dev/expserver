...
void listener_connection_handler(void* ptr) {
    ...
    while (1) {
        ...
        if(listener->port == 8001){
            ...
            logger(LOG_DEBUG, "proxy", "connecting to upstream 127.0.0.1:3000");
            if (upstream == NULL) {
                logger(LOG_ERROR, "listener_connection_handler()", "upstream connection failed");
                xps_connection_destroy(client);  // or close(conn_sock_fd)
                return;
            }
            ...
            int total_read = 0;
            while (1) {
                    ssize_t peek = recv(conn_sock_fd, file_path, sizeof(file_path), MSG_PEEK);
                    ...
                    if (peek < 0) {
                        if (errno == EAGAIN || errno == EWOULDBLOCK) {
                            ...
                            logger(LOG_ERROR, "listener_connection_handler()", "recv peek failed");
                ...
                while (1) {
                    ssize_t bytes_read = recv(conn_sock_fd,
                                            file_path + total_read,
                                            sizeof(file_path) - 1 - total_read,
                                            0);
                    ...
                    if (bytes_read < 0) {
                        if (errno == EAGAIN || errno == EWOULDBLOCK) {
                            ...
                            logger(LOG_ERROR, "listener_connection_handler()", "recv() failed");
                            perror("recv error");
                            close(conn_sock_fd);
                            return;
                        }
                    ...
                    if (total_read >= sizeof(file_path) - 1) {
                        break;
                    }
                }
                ...
                file_path[total_read] = '\0';

                char *newline = strchr(file_path, '\n');
                if (newline) *newline = '\0';

                logger(LOG_DEBUG, "8003", "Requested file: %s", file_path);

                int error;
                xps_file_t *file = xps_file_create(listener->core, file_path, &error);
                if (file == NULL) return;

                xps_pipe_create(listener->core,
                                DEFAULT_PIPE_BUFF_THRESH,
                                file->source,
                                client->sink);
        }
        else {
